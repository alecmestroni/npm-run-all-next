...
describe('[sequencial] npm-run-all', () => {
  ...
  describe('should run tasks sequentially:', () => {
    ...
    it('npm-run-all command --sequential', async () => {
      await runAll(['--sequential', 'test-task:append a', 'test-task:append b'])
      assert.strictEqual(result(), 'aabb')
    })

    it('npm-run-all command --serial', async () => {
      await runAll(['--serial', 'test-task:append a', 'test-task:append b'])
      assert.strictEqual(result(), 'aabb')
    })

    it('npm-run-all command -s', async () => {
      await runAll(['-s', 'test-task:append a', 'test-task:append b'])
      assert.strictEqual(result(), 'aabb')
    })
