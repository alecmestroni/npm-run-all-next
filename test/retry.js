 ...
 * @license MIT
 *
 * Tests for the --retries N option. This flag makes a failing task be retried
...
describe("[retries] npm-run-all", () => {
  ...
  describe("[retries]", () => {
    describe("should not retries when task succeeds on first attempt:", () => {
      ...
      it("Node API", async () => {
        ...
        const results = await nodeApi(["test-task:append1 a"], { retries: retries })
      ...
      it("npm-run-all command", async () => {
        await runAll(["--retries", retries, "test-task:append1 a"])
        assert.strictEqual(result(), "a")
      })

      it("run-p command", async () => {
        await runPar(["--retries", retries, "test-task:append1 a"])
        assert.strictEqual(result(), "a")
      })
    })
    ...
    describe("should retries a task up to N times before giving up:", () => {
      ...
      it("Node API", async () => {
        try {
          await nodeApi([`test-task:append1Error a`], { retries: retries })
        } catch (err) {
      ...
      it("npm-run-all command", async () => {
        try {
          await runAll(["--retries", retries, `test-task:append1Error a`])
        } catch (err) {
          assert.strictEqual(result().length, retries + 1)
          return
        }
      ...
      it("npm-p command", async () => {
        try {
          await runPar(["--retries", retries, `test-task:append1Error a`])
        } catch (err) {
          assert.strictEqual(result().length, retries + 1)
          return
        }
      ...
      it("run-s command", async () => {
        try {
          await runSeq(["--retries", retries, `test-task:append1Error a`])
        } catch (err) {
          assert.strictEqual(result().length, retries + 1)
          return
        }
    ...
    describe("should eventually succeed if failures < retries count:", () => {
      it("Node API", async () => {
        const threshold = 1
        const results = await nodeApi([`test-task:flaky ${threshold}`], { retries: 4 })
        assert.strictEqual(results[0].code, 0)
      ...
      it("npm-run-all command", async () => {
        const threshold = 2
        await runAll(["--retries", "3", `test-task:flaky ${threshold}`])
        assert.strictEqual(result().length, threshold + 1)
      })

      it("run-p command", async () => {
        const threshold = 3
        await runPar(["--retries", "3", `test-task:flaky ${threshold}`])
        assert.strictEqual(result().length, threshold + 1)
      })

      it("run-s command", async () => {
        const threshold = 3
        await runSeq(["--retries", "3", `test-task:flaky ${threshold}`])
        assert.strictEqual(result().length, threshold + 1)
      })
    })

    describe("should error on invalid retries count:", () => {
      it("Node API", async () => {
        try {
          await nodeApi(["test-task:append a", "test-task:append b", "test-task:append c"], { parallel: true, retries: "a" })
        } catch (err) {
          assert.ok(/Invalid options.retries/i.test(err.message))
        }
      })
      it("npm-run-all command --retries '-1'", async () => {
        try {
          await runAll(["--retries", "-1", "test-task:append1 a"])
        } catch (err) {
          assert.ok(/ERROR: Invalid Option: --retries/i.test(err.message))
          return
        }
        assert.fail("Expected an error about invalid retries count")
      })
      it("npm-run-all command --retries -1", async () => {
        try {
          await runAll(["--retries", -1, "test-task:append1 a"])
      ...
      it("npm-run-all command --retries a", async () => {
        try {
          await runAll(["--retries", "a", "test-task:append1 a"])
        } catch (err) {
          assert.ok(/ERROR: Invalid Option: --retries/i.test(err.message))
          return
        }
      ...
      it("npm-run-all command --retries 0", async () => {
        try {
          await runAll(["--retries", 0, "test-task:append1 a"])
        } catch (err) {
          assert.ok(/ERROR: Invalid Option: --retries/i.test(err.message))
          return
        }
    ...
    describe("should kill child processes when killed during retries:", () => {
      ...
      it("run-p command", async () => {
        await spawnWithKill("node", ["../bin/run-p/index.js", "--retries", "5", "test-task:append2 a"])
        assert.ok(result() === null || result() === "a")
      })
    })
  })

  describe("[retries + race]", () => {
    ...
    describe("should combine retries with race execution (abort):", () => {
      ...
      it("Node API", async () => {
        try {
          await nodeApi([`test-task:append1Error a`, `test-task:append2Error b`], { retries: retries, parallel: true, race: true })
        } catch (err) {
      ...
      it("npm-run-all command", async () => {
        try {
          await runAll(["--retries", retries, "--race", "--parallel", `test-task:append1Error a`, "test-task:append2Error b"])
        } catch (err) {
          await delay(1500)
          assert.ok(["aba", "baa"].includes(result()), `Expected result to be one of 'aba', 'baa' but got "${result()}"`)
          return
      ...
      it("npm-p command", async () => {
        try {
          await runPar(["--retries", retries, "--race", `test-task:append1Error a`, "test-task:append2Error b"])
        } catch (err) {
          await delay(1500)
          assert.ok(["aba", "baa"].includes(result()), `Expected result to be one of 'aba', 'baa' but got "${result()}"`)
          return
  ...
  describe("[retries + sequential]", () => {
    ...
    describe("should combine retries with sequential execution (flaky):", () => {
      ...
      const threshold = 2
      it("Node API", async () => {
        const results = await nodeApi([`test-task:flaky ${threshold}`, `test-task:append1 b`], { retries: retries, parallel: false })
        assert.strictEqual(results[0].code, 0)
        assert.strictEqual(results[1].code, 0)
        assert.strictEqual(results[0].name, `test-task:flaky ${threshold}`)
        assert.strictEqual(results[1].name, "test-task:append1 b")
      ...
      it("npm-run-all command", async () => {
        await runAll(["--retries", retries, `test-task:flaky ${threshold}`, `test-task:append1 b`])
        assert.strictEqual(result(), "fffb")
      })

      it("run-s command", async () => {
        await runSeq(["--retries", retries, `test-task:flaky ${threshold}`, `test-task:append1 b`])
        assert.strictEqual(result(), "fffb")
      })
    })
    ...
    describe("should combine retries with sequential execution (abort):", () => {
      ...
      it("Node API", async () => {
        try {
          await nodeApi([`test-task:append1Error a`, `test-task:append1Error b`], { retries: retries, parallel: false })
        } catch (err) {
          assert.strictEqual(err.results.length, 2)
          assert.strictEqual(err.results[0].name, "test-task:append1Error a")
      ...
      it("npm-run-all command", async () => {
        try {
          await runAll(["--retries", retries, `test-task:append1Error a`, `test-task:append1Error b`])
        } catch (err) {
          assert.strictEqual(result(), "aa")
          return
        }
      ...
      it("npm-s command", async () => {
        try {
          await runSeq(["--retries", retries, `test-task:append1Error a`, `test-task:append1Error b`])
        } catch (err) {
          assert.strictEqual(result(), "aa")
          return
        }
    ...
    describe("should combine retries with sequential execution with continueOnError flag:", () => {
      ...
      it("Node API", async () => {
        try {
          await nodeApi([`test-task:append1Error a`, `test-task:append1Error b`], { retries: retries, parallel: false, continueOnError: true })
        } catch (err) {
          assert.strictEqual(err.results.length, 2)
          assert.strictEqual(err.results[0].name, "test-task:append1Error a")
          assert.strictEqual(err.results[1].name, "test-task:append1Error b")
      ...
      it("npm-run-all command", async () => {
        try {
          await runAll(["--retries", retries, "--continue-on-error", `test-task:append1Error a`, `test-task:append1Error b`])
        } catch (err) {
          assert.strictEqual(result(), "aabb")
          return
        }
      ...
      it("npm-s command", async () => {
        try {
          await runSeq(["--retries", retries, "--continue-on-error", `test-task:append1Error a`, `test-task:append1Error b`])
        } catch (err) {
          assert.strictEqual(result(), "aabb")
          return
        }
  ...
  describe("[retries + parallel]", () => {
    describe("should combine retries with parallel execution (succeed):", () => {
      const retries = 1
      it("Node API", async () => {
        const results = await nodeApi([`test-task:append1 a`, `test-task:append1 b`], { retries: retries, parallel: true })
        assert.strictEqual(results[0].code, 0)
        assert.strictEqual(results[1].code, 0)
        assert.strictEqual(results[0].name, "test-task:append1 a")
        assert.strictEqual(results[1].name, "test-task:append1 b")
      ...
      it("npm-run-all command", async () => {
        await runAll(["--retries", retries, "--parallel", `test-task:append1 a`, `test-task:append1 b`])
        assert.ok(["a", "ab", "ba"].includes(result()), `Expected result to be one of "a", "ab", "ba" but got "${result()}"`)
      })

      it("run-p command", async () => {
        await runPar(["--retries", retries, `test-task:append1 a`, `test-task:append1 b`])
        assert.ok(["a", "ab", "ba"].includes(result()), `Expected result to be one of "a", "ab", "ba" but got "${result()}"`)
      })
    })
    ...
    describe("should combine retries with parallel execution (flaky + fail):", () => {
      ...
      it("Node API", async () => {
        try {
          await nodeApi([`test-task:flaky ${threshold}`, `test-task:append1Error b`], { retries: retries, parallel: true })
        } catch (err) {
          assert.strictEqual(err.results.length, 2)
      ...
      it("npm-run-all command", async () => {
        try {
          await runAll(["--retries", retries, "--parallel", `test-task:flaky ${threshold}`, `test-task:append1Error b`])
        } catch (err) {
          await delay(1500)
          assert.strictEqual((result().match(/b/g) || []).length, retries + 1)
          assert((result().match(/f/g) || []).length <= retries)
      ...
      it("run-p command", async () => {
        try {
          await runPar(["--retries", retries, `test-task:flaky ${threshold}`, `test-task:append1Error b`])
        } catch (err) {
          await delay(1500)
          assert.strictEqual((result().match(/b/g) || []).length, retries + 1)
          assert((result().match(/f/g) || []).length <= retries)
    ...
    describe("should combine retries with parallel execution (flaky + succeed):", () => {
      ...
      const threshold = 2
      it("Node API", async () => {
        const results = await nodeApi([`test-task:flaky ${threshold}`, `test-task:append1 b`], { retries: retries, parallel: true })
        assert.strictEqual(results[0].code, 0)
        assert.strictEqual(results[1].code, 0)
        assert.strictEqual(results[0].name, `test-task:flaky ${threshold}`)
        assert.strictEqual(results[1].name, "test-task:append1 b")
      ...
      it("npm-run-all command", async () => {
        await runAll(["--retries", retries, "--parallel", `test-task:flaky ${threshold}`, `test-task:append1 b`])
        assert.ok(["fbff", "bfff"].includes(result()), `Expected result to be one of 'fbff', 'bfff' but got "${result()}"`)
      })

      it("run-p command", async () => {
        await runPar(["--retries", retries, `test-task:flaky ${threshold}`, `test-task:append1 b`])
        assert.ok(["fbff", "bfff"].includes(result()), `Expected result to be one of 'fbff', 'bfff' but got "${result()}"`)
      })
    })
    ...
    describe("should combine retries with parallel execution and jobs:", () => {
      ...
      it("run-p command", async () => {
        await runPar(["--retries", retries, "--jobs", jobs, `test-task:flaky ${threshold}`, `test-task:append1 a`, `test-task:append2 b`])
        assert.ok(["fabfb", "afbfb"].includes(result()), `Expected one of fabfb|afbfb but got "${result()}"`)
      })
    })
  })

  describe("[retries + aggregated-output]", () => {
    const retries = 3
    const EXPECTED = [
      ...createExpectedOutput("second", retries),
      ...createExpectedOutput("third", retries),
      ...createExpectedOutput("first", retries),
      "",
    ].join("\n")
    ...
    describe("should fail without parallel flag:", () => {
      it("Node API", async () => {
        try {
          await nodeApi(["test-task:delayed:flaky first 500"], { stdout, retries: 2, silent: true, aggregateOutput: true })
        } catch (err) {
      ...
      it("npm-run-all command", async () => {
        try {
          await runAll(["--retries", "2", "--aggregate-output", "--silent", "test-task:delayed:flaky first 100"], stdout)
        } catch (err) {
      ...
      it("run-s command", async () => {
        try {
          await runSeq(["--retries", "2", "--aggregate-output", "--silent", "test-task:delayed:flaky first 100"], stdout)
        } catch (err) {
          assert.ok(/aggregate-output/.test(err.message))
          return
        }
  ...
  describe("[retries + silent]", () => {
    describe('"should not print log if silent option was given:', () => {
      const retries = 3
      it("Node API command", async () => {
        try {
          await nodeApi(["test-task:append1Error b"], { retries: retries, silent: true })
        } catch (err) {
      ...
      it("npm-run-all command", async () => {
        try {
          await runAll(["--silent", "--retries", retries, "test-task:append1Error d"])
        } catch (err) {
      ...
      it("run-p command", async () => {
        try {
          await runPar(["--silent", "--retries", retries, "test-task:append1Error e"])
        } catch (err) {
      ...
      it("run-s command", async () => {
        try {
          await runSeq(["--silent", "--retries", retries, "test-task:append1Error f"])
        } catch (err) {
