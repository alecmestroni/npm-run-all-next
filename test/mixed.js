...
describe('[mixed] npm-run-all', () => {
  ...
  it('should run a mix of sequential and parallel tasks (has the default group):', async () => {
    ...
    assert.ok(
      ...
      `Expected result to be one of "aabcbcddee", "aabccbddee", "aacbbcddee", "aacbcbddee" but got "${result()}"`
    )
  })
  ...
  it("should run a mix of sequential and parallel tasks (doesn't have the default group):", async () => {
    ...
    assert.ok(
      ['bcbcddee', 'bccbddee', 'cbbcddee', 'cbcbddee'].includes(result()),
      `Expected result to be one of "bcbcddee", "bccbddee", "cbbcddee", "cbcbddee" but got "${result()}"`
    )
  })
