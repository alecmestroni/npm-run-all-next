...
module.exports = class NpmRunAllError extends Error {
  ...
  constructor(causeResult, allResults) {
    const message = causeResult.message
      ?? `"${causeResult.task ?? causeResult.name}" exited with ${causeResult.code}.`
    super(message)
    ...
    /**
     * The invocationId of the task that caused the error, if tracked.
     * @type {string|null}
     */
    this.invocationId = causeResult.invocationId || null
  }
}
