...
function detectConcurrencyWeight(scriptCmd, taskList) {
  // Supports the legacy and branded CLI names, plus direct bin entrypoints.
  const isRunPCommand = /run[- ]p /.test(scriptCmd) || /\.\.\/bin\/run-p\/index\.js/.test(scriptCmd)
  const isParallelRunAllCommand =
    ((/npm[- ]run[- ]all(?:[- ]next)? /.test(scriptCmd) || /\.\.\/bin\/npm-run-all(?:-next)?\/index\.js/.test(scriptCmd)) &&
      /(^|\s)(-p|--parallel)(\s|$)/.test(scriptCmd))

  if (isRunPCommand || isParallelRunAllCommand) {
    try {
      const scriptCmdCleaned = scriptCmd
        .replace(/node\s+\.\.\/bin\/npm-run-all(?:-next)?\/index\.js /, "")
        .replace(/node\s+\.\.\/bin\/run-p\/index\.js /, "")
        .split(" ")
        .filter((s) => s !== "run-p" && s !== "npm-run-all" && s !== "npm-run-all-next")
      const args = isRunPCommand
        ? parseCliArgs(scriptCmdCleaned, { parallel: true }, { singleMode: true })
        : parseCliArgs(scriptCmdCleaned)
      if (args.jobs > 0) {
        return args.jobs
      } else {
        let totalTasks = 0
