...
/**
 * Read historical runtimes from JSON file.
 * @returns {Object} map of scriptName -> { count, avgRuntime }
 */
function readRuntimes() {
  const p = path.resolve(process.cwd(), '.npm-run-all-next-runtimes.json')
  if (!fs.existsSync(p)) return {}
  try {
    return JSON.parse(fs.readFileSync(p, 'utf8'))
  } catch (err) {
    console.warn('Could not parse runtimes file, starting fresh:', err)
    return {}
  }
}
...
function detectConcurrencyWeight(scriptCmd, taskList) {
  ...
  if ((/npm[- ]run[- ]all /.test(scriptCmd) && /--parallel/.test(scriptCmd)) || /run[- ]p /.test(scriptCmd)) {
    try {
      ...
      if (args.maxParallel > 0) {
        ...
        return totalTasks > 0 ? totalTasks : 2 // oppure il valore che preferisci come default
      }
    } catch (e) {
      console.warn('Error parsing script command:', scriptCmd, e)
    }
  }
