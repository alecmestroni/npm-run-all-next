 ...
 *
 * Trailing `:**` is treated as "zero or more additional segments" so that
 * e.g. `test:gateway:plugins:!(ai):**` matches both `test:gateway:plugins:cors`
 * and `test:gateway:plugins:cors:unit`.  Minimatch's native `/**` requires at
 * least one extra segment; we rewrite the trailing `/**` to `{,/**}` to also
 * match the parent level.
 *
...
function createFilter(pattern) {
  ...
  let globTask = swapColonAndSlash(task)

  // Trailing /** should match zero or more levels (not just one or more).
  if (globTask.endsWith("/**")) {
    globTask = globTask.slice(0, -3) + "{,/**}"
  }

  const matcher = new Minimatch(globTask, { nonegate: true })
