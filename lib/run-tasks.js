...
module.exports = function runTasks(tasks, options) {
  ...
  let results = tasks.map((task) => ({ name: task.name, code: undefined, retries: 0, durationMs: 0, completionOrder: Infinity }))
  let completionSeq = 0
  ...
  return new Promise((resolve, reject) => {
    ...
    function done() {
      ...
      if (showSummary && options.stdout) {
        const sorted = [...results].sort((a, b) => a.completionOrder - b.completionOrder)
        options.stdout.write(printSummaryTable(sorted, Date.now() - startTime, jobs))
      }
      if (error != null) {
        // Rebuild a coherent final error from all results
        const failures = results.filter((r) => r.code && r.code !== 0 && r.code !== 137)
        if (failures.length > 1) {
          const detailMsg = generateErrorMessage(failures)
          error = new NpmRunAllError(
            { message: `${failures.length} tasks failed`, code: 1 },
            results
          )
          // Print error details to stdout (after the table) to avoid
          // stderr/stdout interleaving that breaks the table layout.
          if (options.stdout) options.stdout.write('\n' + detailMsg)
          error.reported = true
          ...
          // error was set (e.g. by catch handler) but no matching failures in results
