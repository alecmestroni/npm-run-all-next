  ...
  return new Promise((resolve, reject) => {
    ...
    function done() {
      if ((options.balancer || options.runtimeFile) && !alreadyUpdatedRuntimesFile) {
        ...
        let extraMeasurements = null
        if (options.parallel && options.parallelGroupName) {
          const allSucceeded = results.every((r) => r.code === 0)
          if (allSucceeded) {
            const totalSecs = (Date.now() - startTime) / 1000
            extraMeasurements = { [options.parallelGroupName]: [totalSecs] }
          }
        }
        updateHistoryFromResults(results, options.runtimeFile, extraMeasurements)
      }
