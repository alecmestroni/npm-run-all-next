...
const { ENV_FILE, ENV_ROOT, ENV_PARENT, createId } = require("./summary-report")
 ...
 * @param {boolean} options.aggregateTable
...
module.exports = function npmRunAll(patternOrPatterns, options) {
  ...
  const aggregateTable = Boolean(options.aggregateTable)
  ...
  const inheritRetries = Boolean(options.inheritRetries)
  const retryCountToPropagate = Number(options.retryCountToPropagate) || retries
  ...
  const parallelGroupName = parallel
    ? Array.isArray(patternOrPatterns)
      ? patternOrPatterns.join(" ")
      : String(patternOrPatterns)
    : null

  // Summary tracing context for unified parent/child reporting.
  const envSummaryFile = process.env[ENV_FILE] || null
  const envSummaryRootId = process.env[ENV_ROOT] || null
  const envSummaryParentId = process.env[ENV_PARENT] || null

  let summaryReportFile = options.summaryReportFile || envSummaryFile || null
  const summaryRootId = options.summaryRootId || envSummaryRootId || createId("root")
  const summaryParentInvocationId = options.summaryParentInvocationId || envSummaryParentId || null
  const summaryAutoCleanup = options.summaryAutoCleanup !== false

  if (!summaryReportFile && printSummaryTable) {
    summaryReportFile = path.join(os.tmpdir(), `npm-run-all-next-summary-${summaryRootId}-${process.pid}.jsonl`)
  }
  ...
  try {
    ...
    if (!aggregateOutput && aggregateTable) {
      throw new Error('Invalid options.aggregateTable; It requires aggregateOutput');
    }
    if (!parallel && race) {
