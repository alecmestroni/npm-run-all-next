...
function buildHierarchicalRows(results, options) {
  ...
  function visit(node, prefix, isLast, hasParent) {
    if (seen.has(node)) return
    seen.add(node)
    const indent = hasParent ? `${prefix}${isLast ? "└── " : "├── "}` : ""
    ordered.push({
      name: `${indent}${withModeLabel(node.name, node.mode, Boolean(opts.showMode) && hasParent)}`,
      code: node.code,
    ...
    const childPrefix = hasParent ? `${prefix}${isLast ? "    " : "│   "}` : ""
    children.forEach((child, idx) => {
      visit(child, childPrefix, idx === children.length - 1, true)
    })
  }

  roots.forEach((node) => visit(node, "", true, false))

  // Include any remaining top-level rows (defensive fallback).
  nodes.forEach((node) => {
    if (!seen.has(node) && !node.parentInvocationId) {
      ordered.push({
        name: withModeLabel(node.name, node.mode, false),
        code: node.code,
        retries: node.retries,
        durationMs: node.durationMs,
      })
...
module.exports = function printSummaryTable(results, totalTime, jobs, options) {
  ...
  const headers = ["Task", "FinalExitCode", "Retries", "Time"]
  const rows = sorted.map(({ name, code, retries, durationMs }) => {
    code = code == 137 ? code + " (Killed)" : code
    return [name, String(code), String(retries), formatTime(durationMs / 1000)]
  })
  ...
  if (typeof jobs === "number" && jobs > 0) {
    ...
    output += "|" + jobsLabel + " ".repeat(totalInner - jobsLabel.length - jobsStr.length) + jobsStr + "|\n"
  }

  // Estimated Total Time (only when parallel, i.e. jobs > 1, to avoid redundancy with Actual)
  if (typeof jobs === "number" && jobs > 1) {
    const estResults = opts.hierarchical
      ? results.filter((r) => !r.parentInvocationId)
      : results
    const estTotalMs = estResults.reduce((sum, r) => sum + r.durationMs, 0)
    const estTimeStr = formatTime(estTotalMs / 1000)
    const estLabel = " Estimated Total Time: "
    output += "|" + estLabel + " ".repeat(totalInner - estLabel.length - estTimeStr.length) + estTimeStr + "|\n"
  }
  ...
  if (typeof totalTime === "number") {
    ...
    output += "|" + actLabel + " ".repeat(totalInner - actLabel.length - actualTimeStr.length) + actualTimeStr + "|\n"
  }

  // Start time (always shown when available)
  if (opts.startTime) {
    const startTimeStr = new Date(opts.startTime).toISOString()
    const startLabel = " Start time: "
    output += "|" + startLabel + " ".repeat(totalInner - startLabel.length - startTimeStr.length) + startTimeStr + "|\n"
  }

  // Final table closure
  output += divider + "\n"

  // Error summary after the table — use sorted (the visible table rows) to avoid
  // including children from superseded (retried) group executions.
  const stripPrefix = (n) => n.replace(/^(?:[│ ]*[├└]── )?(?:\[[SP]\] )?/, "")
  const failures = sorted
    .filter((r) => r.code && r.code !== 0 && r.code !== 137)
    .map((r) => ({ name: stripPrefix(r.name), code: r.code }))
  if (failures.length > 1) {
    const byCode = failures.reduce((acc, { name, code }) => {
      ;(acc[code] ||= []).push(name)
      return acc
    }, {})
    output += `\n🚨 ${failures.length} scripts failed:\n\n`
    for (const [code, names] of Object.entries(byCode)) {
      output += `${names.length} exited with code ${code}:\n`
      output += `-----\n`
    ...
    output += `\nERROR: "${failures[0].name}" exited with ${failures[0].code}.\n`
  }
