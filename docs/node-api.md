| [index](../README.md) | [npm-run-all-next](npm-run-all.md) | [run-s](run-s.md) | [run-p](run-p.md) | Node API |
| --------------------- | ----------------------------- | ----------------- | ----------------- | -------- |
...
```js
const runAll = require("npm-run-all-next")
...
- **options** `object`
  ...
  - **options.packageConfig** `object|null` --
    ...
    e.g. `{"npm-run-all-next": {"test": 777, "test2": 333}}`
    Default is `null`.
...
- If you don't use those options and `process.stdXXX.isTTY` is `false`, please configure max listeners of the `process.stdXXX` properly. In that case, `npm-run-all-next` uses piping to connect to child processes.<br>
  On the other hand, if `process.stdXXX.isTTY` is `true`, `npm-run-all-next` uses `inherit` option, so the configuration is unnecessary.
